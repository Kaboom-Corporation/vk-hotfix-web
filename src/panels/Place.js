...
const Place = ({
  ...
  const formattedPrice = accounting.formatNumber(price, 0, " ");
  ...
  return (
    <div className="Place">
      ...
      <footer className="Place__footer">
        <Link
          to={price ? `/basket/${area.id}/${item.id}` : "#"}
          className={`Place__order ${formattedPrice == 0 ? 'disable' : ""}`}
        >
          {formattedPrice > 0 ? `Добавить в корзину (${formattedPrice})`  : "Корзина пуста"}
        </Link>
      </footer>
    </div>
  );
